...
The closing feature tag is required and must match the opening feature tag.
Lookup blocks differ in that their closing label is optional; see
§[4.e](#4.e).
...
script and language selection. To register a rule or lookup under more than one
script, you must explicitly include it following each script specification.
Multiple languages for the same script can be selected by one `language`
...
##### language statement:
The language selection stays the same until explicitly changed, until the script
is changed, or until the end of the feature. To change the language selection,
use the `language` statement:

```fea
language <language tag> (<language tag>)*
    [exclude_dflt|include_dflt] [required];
```

One or more language tags may be specified. Subsequent rules and lookup
references are registered under every listed language for the current script.
The `dflt` language tag must be specified alone.

For example, the following statement selects five language systems without
requiring the same rules to be repeated five times:
...
The compiler may share the lookup and feature data generated by the following
rules among all the listed languages. This permits more compact output than
repeating the rules after separate `language` statements.
...
As a result of this statement, (a) the language selection is changed to `DEU `,
and (b) the current default lookups are automatically included into the language
system specified by the current script and language DEU. When multiple language
tags are listed, these effects apply to every listed language.
...
An empty `lookupflag` statement resets the lookup flag value to 0 without
setting any named flags. It is equivalent to `lookupflag 0;`.
...
```fea
[standalone] lookup <label> [useExtension] {
    # rules to be grouped
} [<label>];
```
...
defined outside a feature block, it is referred to as a ‘standalone’ lookup.

Normally, a lookup defined inside a feature block is both defined and
immediately registered under the feature’s current language systems. The
optional `standalone` keyword prevents that immediate registration. The lookup
is used only when it is referenced by a `lookup` statement or by a contextual
rule. This permits a secondary lookup to be defined next to the contextual rule
that uses it while preserving lookup definition order:

```fea
feature ccmp {
    standalone lookup CASE_ACCENTS {
        substitute gravecomb by gravecomb.case;
        substitute acutecomb by acutecomb.case;
    };

    substitute @Uppercase @TopAccents' lookup CASE_ACCENTS;
} ccmp;
```

The `standalone` keyword is redundant on a top-level lookup definition and is
ignored there.

The closing label after `}` is optional. When present, it must match the
opening label. Omitting the closing label does not change the lookup's name or
scope:
...
Most glyph substitution rules begin with the keyword `substitute`; this keyword
may be abbreviated as `sub`. (The `ignore` keyword may precede the `substitute`
keyword in some cases.) Glyph deletion can instead begin with `delete` or
`del`; see §[5.i](#5.i). The GSUB lookup type is auto-detected from the format
...
substitute <glyphclass> by <glyph-or-glyphclass sequence>;  # format C
```

The replacement sequence contains two or more elements. Format A replaces the
target glyph with a sequence of glyphs or singleton glyph classes.

Format B replaces every glyph in the target glyph class with the same
replacement sequence.

Format C creates one replacement sequence for each glyph in the target glyph
class. Each replacement class must contain either one glyph or the same number
of glyphs as the target class. The glyphs are paired by their order in the
classes. A glyph or singleton replacement class is repeated in every generated
...
The third example produces an identical representation in the font as:

```fea
substitute f_i by f i;
substitute f_l by f l;
substitute T_h by T h;
```
...
If there is only a single substitution operation, and it is a single, multiple,
or ligature substitution, then the operation can be specified in-line and its
type will be auto-detected from the input and replacement sequence in the same
way as in the corresponding standalone (i.e. non-contextual) statement.
...
###### Example 3a:
This calls a Multiple Sub lookup. The rule below means: when a symbol occurs
between a member of `@Left` and a member of `@Right`, replace it with its
corresponding pair of component glyphs. The marked input and replacement classes
follow the class-length and singleton-repetition rules in §[5.b](#5.b).

```fea
substitute @Left @Symbol' @Right by @SymbolLeft @SymbolRight;
```

This format requires that there be only one marked glyph or glyph class in the
input sequence.
...
The `delete` statement removes a glyph from the glyph sequence. The keyword may
be abbreviated as `del`. A non-contextual deletion is specified as:

```fea
delete <glyph|glyphclass>;
```
...
Deletion can also be contextual. Exactly one glyph or glyph class must be
marked for deletion; unmarked glyphs and glyph classes provide the backtrack
and lookahead context:

```fea
delete <backtrack glyph sequence>*
       <marked glyph|glyphclass>
       <lookahead glyph sequence>*;
```

For example, the following deletes `tonos` only when it is followed by
`Alpha`:
...
A `delete` statement is equivalent to a substitution of the marked glyph or
glyph class by an empty replacement. The examples above may therefore also be
written using `NULL`:

```fea
substitute tonos by NULL;
substitute @Deprecated by NULL;
substitute tonos' Alpha by NULL;
```

A compiler implements a non-contextual deletion with a Multiple Substitution
lookup whose replacement sequence is empty. A contextual deletion uses such a
lookup as the secondary lookup at the marked input position.
